...
contract('contractProxyEthBondingToken', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    if (trace)
      console.log('SUCESSO: ERC20BondingToken.new({from: accounts[0]}');
    contractTestEthBondingToken = await TestEthBondingToken.new({
      from: accounts[0],
    });
    if (trace)
      console.log('SUCESSO: TestEthBondingToken.new({from: accounts[0]}');
    contractTestERC20BondingToken = await TestERC20BondingToken.new({
      from: accounts[0],
    });
    if (trace)
      console.log('SUCESSO: TestERC20BondingToken.new({from: accounts[0]}');
    ...
    contractProxyEthBondingToken = await ProxyEthBondingToken.new({
      from: accounts[0],
    });
  });
  ...
  it('Should fail test_curvedMint() when NOT comply with: msg.value > 0', async () => {
    let result = await truffleAssert.fails(
      contractProxyEthBondingToken.test_curvedMint({from: accounts[3]}),
      'revert',
    );
  });
  it('Should fail test_curvedMint() when NOT comply with: msg.gasprice <= gasPrice', async () => {
    let result = await truffleAssert.fails(
      contractProxyEthBondingToken.test_curvedMint({
        from: accounts[3],
        value: 89,
      }),
      'revert',
